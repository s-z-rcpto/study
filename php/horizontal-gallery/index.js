...
function openModal(imageNumber, imageUrl, imageAlt) {
  ...
  // Отримуємо велике зображення з об'єкта
  const imageObj = getImageObj(imageNumber);
  const largeImageUrl = imageObj.src_large || imageObj.src;

  const img = document.createElement("img");

  img.addEventListener("load", function () {
    modalImage.innerHTML = `<img src="${largeImageUrl}" alt="${imageAlt}" title="${imageAlt}" />`;
  });
